    ...
    <>
      ...
      <Section className="!pt-10">
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-[var(--color-violet)] bg-[color-mix(in_srgb,var(--color-violet)_10%,transparent)] p-6">
          ...
          {c.secondaryMetrics.map((m) => (
            <div key={m.label} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
              <div className="text-3xl font-bold text-[var(--color-ink)]">{m.value}</div>
              <p className="mt-1 text-sm text-[var(--color-ink-dim)]">{m.label}</p>
            </div>
          ))}
        </div>
      </Section>
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_320px]">
          <div className="space-y-10">
            <div>
              ...
              <p className="mt-5 text-[var(--color-ink-dim)]">{c.challenge}</p>
            </div>
            <div>
            ...
            <figure className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-7">
              ...
              <figcaption className="mt-4 text-sm text-[var(--color-ink-faint)]">— {c.quote.author}, {c.quote.role}</figcaption>
            </figure>

            <p className="rounded-xl border border-[var(--color-border)] bg-[var(--color-base-2)] p-4 text-xs text-[var(--color-ink-faint)]">{REPRESENTATIVE_DISCLOSURE}</p>
          </div>

          <aside className="h-fit space-y-4">
            <div className="rounded-2xl border border-[var(--color-border-bright)] bg-[var(--color-surface)] p-6">
              <h3 className="font-semibold">Want results like these?</h3>
              <p className="mt-2 text-sm text-[var(--color-ink-dim)]">Get a free audit of your accounts and a realistic projection for your business.</p>
              <Button href="/contact" className="mt-5 w-full">Get a free audit <ArrowRight size={16} /></Button>
            </div>
            <Link href={`/industries/${c.industrySlug}`} className="block rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 transition-colors hover:border-[var(--color-cyan)]">
              <p className="text-sm text-[var(--color-ink-faint)]">Industry</p>
              <p className="mt-1 font-semibold text-[var(--color-cyan-bright)]">{c.industry} marketing →</p>
            </Link>
          </aside>
        </div>
      </Section>
      ...
      <Section className="bg-[var(--color-base-2)]">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {others.map((o) => (
            <Link key={o.slug} href={`/results/${o.slug}`} className="group rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 transition-all hover:-translate-y-1 hover:border-[var(--color-violet)]">
              <div className="text-4xl font-bold text-gradient">{o.headlineMetric.value}</div>
              <p className="mt-1 text-sm text-[var(--color-ink-dim)]">{o.headlineMetric.label}</p>
              <p className="mt-4 text-sm text-[var(--color-ink-faint)]">{o.industry} · {o.location}</p>
            </Link>
          ))}
        </div>
      </Section>
