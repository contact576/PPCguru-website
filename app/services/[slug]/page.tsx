      ...
      <Section>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {service.outcomes.map((o) => (
            <div key={o} className="flex items-center gap-3 rounded-xl border border-[--color-border] bg-[--color-surface] px-5 py-4">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[color-mix(in_srgb,var(--color-success)_18%,transparent)] text-[--color-success]"><Check size={15} /></span>
          ...
          {service.deliverables.map((d, i) => (
            <Reveal key={d.title} delay={(i % 2) * 0.05}>
              <div className="h-full rounded-2xl border border-[--color-border] bg-[--color-surface] p-7">
                ...
                <p className="mt-2 text-sm text-[--color-ink-dim]">{d.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <Section>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {service.process.map((p) => (
            <div key={p.step} className="rounded-2xl border border-[--color-border] bg-[--color-surface] p-6">
              ...
              <p className="mt-1.5 text-sm text-[--color-ink-dim]">{p.body}</p>
            </div>
          ))}
        </div>
      </Section>
        ...
        <Section className="bg-[--color-base-2]">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {relatedIndustries.map((ind) => (
              <Link key={ind.slug} href={`/industries/${ind.slug}`} className="rounded-full border border-[--color-border] bg-[--color-surface] px-5 py-2.5 text-sm transition-colors hover:border-[--color-cyan] hover:text-[--color-cyan-bright]">
                {ind.name}
              </Link>
            ))}
          </div>
