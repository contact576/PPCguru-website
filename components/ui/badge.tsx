...
function LeakMark() {
  return (
    <svg width="44" height="12" viewBox="0 0 44 12" fill="none" aria-hidden className="shrink-0">
      ...
      <path d="M17 7 H34 l 5 -4.5" stroke="var(--color-olive,#6f7d22)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx="15" cy="7" r="2.8" fill="var(--color-ink)" />
      <circle cx="40" cy="2.5" r="3" fill="var(--color-lime)" stroke="var(--color-olive,#6f7d22)" strokeWidth="1" />
    </svg>
  );
}

/** Mono uppercase eyebrow label used above section headings. The leak→sealed
 *  mark is the page's recurring structural signature. */
export function Eyebrow({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <span className={cn("eyebrow inline-flex items-center gap-2.5 text-[var(--color-orange-deep)]", className)}>
      <LeakMark />
      {children}
    </span>
  );
