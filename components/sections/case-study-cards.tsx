      ...
      <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {list.map((c, i) => (
          <Reveal key={c.slug} delay={(i % 3) * 0.06}>
            <Link
              href={`/results/${c.slug}`}
              className="group flex h-full flex-col rounded-2xl border border-[--color-border] bg-[--color-surface] p-7 transition-all duration-300 hover:-translate-y-1 hover:border-[--color-violet]"
            >
              ...
              <p className="mt-5 flex-1 text-sm text-[--color-ink-dim]">{c.client} · {c.service}</p>
              <div className="mt-5 flex items-center gap-1.5 text-sm font-medium text-[--color-cyan-bright]">
                Read the case study
